                ...
                for row_num, row in enumerate(reader, start=1):
                    ...
                    if price_idx is not None and len(row) > price_idx:
                        ...
                        if price is not None:
                            price_source = 'snapshot_imported' if asset.get('valuation_method') == 'snapshot_imported' else 'csv_bootstrap'
                            cursor.execute("UPDATE assets SET current_price = ?, price_source = ?, price_updated_at = ? WHERE id = ?", (float(price), price_source, '2000-01-01', asset['id']))
